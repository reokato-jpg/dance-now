      ...
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          ...
          <div className="card-dark p-5">
            <p className="text-xs font-bold text-ink-400 uppercase tracking-wider mb-2">STUDIO {slot.studioName}</p>
            <h1 className="text-2xl font-bold text-white mb-1">
              {formatTimeRange(slot.startAt, slot.durationMin)}
            </h1>
            <p className="text-sm text-ink-400">{formatDate(slot.startAt, "M月d日(E)")}</p>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="card-dark p-4 text-center">
              ...
              <p className="text-sm font-bold text-white">{slot.durationMin}分</p>
            </div>
            <div className="card-dark p-4 text-center">
              <Users className="w-5 h-5 text-brand-purple mx-auto mb-1" />
              <p className="text-xs text-ink-400 mb-0.5">残り枠</p>
              <p className={cn("text-sm font-bold", available <= 0 ? "text-danger" : available <= 3 ? "text-warning" : "text-white")}>
                {available} / {slot.capacity}
              </p>
            </div>
            <div className="card-dark p-4 text-center">
              <Clock className="w-5 h-5 text-brand-purple mx-auto mb-1" />
              <p className="text-xs text-ink-400 mb-0.5">開始</p>
          ...
          {/* Studio info */}
          <div className="card-dark p-5">
            <h3 className="text-xs font-bold text-ink-400 uppercase tracking-wider mb-3">STUDIO INFO</h3>
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 bg-brand-purple/20 rounded-lg flex items-center justify-center flex-shrink-0">
                <MapPin className="w-5 h-5 text-brand-purple" />
              </div>
              <div>
                <p className="font-bold text-white">Studio {slot.studioName}</p>
                <p className="text-xs text-ink-400 mt-0.5">{slot.studioAddress}</p>
              </div>
            </div>
          </div>
        </motion.div>
